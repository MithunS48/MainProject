...
function PasswordRule({ met, label }) {
  return (
    <div className={`flex items-center gap-1.5 text-xs ${met ? "text-seaweed-400" : "text-white/40"}`}>
      ...
      <div className="min-h-screen relative flex items-center justify-center px-5">
        ...
        >
          <motion.div
            ...
            className="w-20 h-20 rounded-full bg-seaweed-500/20 flex items-center justify-center mx-auto mb-6"
          >
            <CheckCircle2 className="w-12 h-12 text-seaweed-400" />
          </motion.div>
    ...
    <div className="min-h-screen relative flex items-center justify-center px-5 py-16">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-white/60 text-sm mt-1">Start detecting fish disease with AI</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="glass-card rounded-3xl p-8 space-y-5 shadow-glass">
          ...
          {/* Role selector */}
          <div>
            <label className="text-sm font-medium text-white/70 mb-2 block">I am a...</label>
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`rounded-xl p-3.5 border text-left transition-all ${
                  role === "farmer"
                    ? "border-ocean-400 bg-ocean-400/15 shadow-glow-ocean"
                    : "border-white/15 bg-white/5 hover:bg-white/10"
                }`}
              >
                <Sprout className={`w-5 h-5 mb-1.5 ${role === "farmer" ? "text-ocean-300" : "text-white/50"}`} />
                <div className="text-sm font-semibold text-white">Farmer / User</div>
              </button>
              <button
                type="button"
                ...
                className={`rounded-xl p-3.5 border text-left transition-all ${
                  role === "admin"
                    ? "border-coral-400 bg-coral-400/15 shadow-glow-coral"
                    : "border-white/15 bg-white/5 hover:bg-white/10"
                }`}
              >
                <ShieldCheck className={`w-5 h-5 mb-1.5 ${role === "admin" ? "text-coral-300" : "text-white/50"}`} />
            ...
            {role === "admin" && (
              ...
              >
                <label className="text-sm font-medium text-white/70 mb-1.5 block">
                  Admin Invite Code <span className="text-coral-400">*required</span>
                </label>
          ...
          <p className="text-center text-sm text-white/50">
            ...
            <Link to="/login" className="text-ocean-300 font-semibold hover:text-ocean-200">
